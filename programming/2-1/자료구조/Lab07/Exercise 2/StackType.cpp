...
void StackType::Copy(StackType& anotherStack)
{
    ...
    NodeType* anotherTempPtr;

    if (tempPtr != NULL)
    {
        NodeType* anotherTopPtr = new NodeType();
        anotherTopPtr->info = tempPtr->info;
        anotherTopPtr->next = NULL;

        anotherStack.topPtr = anotherTopPtr;

        anotherTempPtr = anotherTopPtr;
        tempPtr = tempPtr->next;

        while (tempPtr != NULL)
        {
            NodeType* newNode = new NodeType();
            newNode->info = tempPtr->info;

            anotherTempPtr->next = newNode;

            anotherTempPtr = newNode;
            tempPtr = tempPtr->next;
        }
        anotherTempPtr->next = NULL;
    } 
}