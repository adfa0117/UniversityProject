...
#pragma warning(disable : 4996) // _CRT_SECURE_NO_WARNINGS으로 에러가 사라지지 않아 추가한 구문입니다.
...
{
    int index = 0;
    RelationType returnValue;

    while (letters[index] != '\0' && otherString.letters[index] != '\0' && letters[index] == otherString.letters[index])
    {
        ++index;
    }
    if (letters[index] == '\0' && otherString.letters[index] == '\0')
    {
        returnValue = RelationType::EQUAL;
    }
    else if (letters[index] == '\0')
    {
        returnValue = RelationType::LESS;
    }
    else if (otherString.letters[index] == '\0')
    {
        returnValue = RelationType::GREATER;
    }
    else
    {
        if (letters[index] < otherString.letters[index])
        {
            returnValue = RelationType::LESS;
        }
        else
